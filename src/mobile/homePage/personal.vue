  ...
  <div class="personal">
    ...
    <div class="introduce">
      ...
      <div class="account">
        ...
        <img src="@/assets/mobile/copy.png" alt="" />
      </div>
      ...
      <div class="handle">
        ...
        <div class="address">
          <img src="@/assets/mobile/my_adress.png" alt="" />
          <span>我的地址</span>
        </div>
      </div>
    </div>
...
<script lang="ts">
import { createList, getPersonalInfo } from "@/api/home";
...
export default {
  ...
  setup() {
    ...
    const getData = async () => {
      const data: any = await getPersonalInfo();
      state.source = data.info;
    };

    const onRefresh = () => {};

    const getList = async () => {
      const res: any = await createList();
      state.leftList = res.info.filter((item: any, index: number) => {
        return index % 2 === 0;
      });
      state.rightList = res.info.filter((item: any, index: number) => {
        return index % 2 === 1;
      });
    };

    const selectCarType = (code: any) => {
